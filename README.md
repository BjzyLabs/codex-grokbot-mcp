...
Implementation is in progress. The current public tree contains the project process, publication checks, the local source and patch contract, and a tested Vault lease client, a scoped GitHub/webhook transport, and a private local job journal. It does not yet provide a working MCP server. Track bounded work in [Beads](.beads/README.md).
...
See [the security model](docs/security-model.md), [local contract](docs/local-contract.md), [Vault setup contract](docs/vault-setup.md), [control transport](docs/control-transport.md), [worker guidance](docs/workers.md), [job journal](docs/job-journal.md), and [contribution guide](CONTRIBUTING.md).
