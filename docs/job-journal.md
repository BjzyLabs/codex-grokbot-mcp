...
Schema v4 records the packet schema, job type, deliver mode, whether a callback was expected, the callback body digest, and a terminal reason. It does not store callback tokens or patches. A ready `x_query` is pinned by that digest instead of a control-repository commit. Schema v3 records the immutable control-repository head commit and a SHA-256 digest of the validated artifact's canonical JSON before a coding job may become `validated` or `ready`. Canonical JSON sorts object keys, uses compact separators and UTF-8, and rejects non-finite numbers. The patch text is never stored in the journal. Result retrieval must read from that pinned commit and compare the digest again before returning a patch. A recorded identity cannot be replaced. Schema-v1 and schema-v2 journals migrate in place; a legacy `validated` or `ready` job without this identity becomes `uncertain` and requires reconciliation.
